...
func filter_transaction(bankTransaction FINANCE_BankTransactions, bankAccount FINANCE_BankAccount) string {

	//O valor 50 se refere ao valor do campo external_id dentro da tabela bank_transaction no Pimenta Finance

	if bankTransaction.External_id != "" {
		return " WHERE external_id='" + bankTransaction.External_id[:50] + "' LIMIT 1 "
	}

	//valor do account_id definido no código do finance fixo em 1
	account_id := "999"

	sql := " WHERE details='" + bankTransaction.Details + "' AND "
	sql += " date='" + bankTransaction.Date + "' AND "
	sql += " account_id=" + account_id + " AND "
	sql += " bank_account_id=" + strconv.Itoa(bankAccount.Id)
	sql += " LIMIT 1 "
...
func GetBankTransaction(transaction FINANCE_BankTransactions, bankAccount FINANCE_BankAccount) (FINANCE_BankTransactions, bool, error) {
